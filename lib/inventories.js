...
var outStockReserve = function(db,location,items,cb){
  checkStock(db,location,items,function(err,items){
    if(err){
      console.log("error in outStockReserve");
      cb(err);
    }
    else{
      console.log("to be asynced");
      async.eachSeries(items,function(item,callback){
        var id = mongoq.mongodb.BSONPure.ObjectID.createFromHexString(item._id);
...
var mergeItem = function(items){
  ...
  for(var i in mergeItems){
    console.log("pushed merge: ",mergeItems[i].quantity);
  ...
  var process_request = function(req,res,next){
    ...
    // console.log(req.body.status.status_code || "no status", "code");
  switch(req.params.object){
    case 'sales' :
        switch(req.body.status.status_code){
          ...
          break;
          default: console.log("no status matched :", req.body.status.status_code );
          next();
          break;
        }
    break;
    case 'consignments' :
      ...
      break;
      default:
        console.log("no status matched :", req.body.status.status_code );
        next();
      }
    break;
    default:
